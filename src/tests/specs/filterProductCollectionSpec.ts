...
import { price } from '../testData/productNames';
...
describe('Filter product collection', function () {
  ...
  it(`Should filter and display products by price from ${price[0]} to ${price[30]}`, function () {
    ...
    productCollectionPage.facetFilter.filterByPriceFromInput.type(`${price[0]}`).wait(Wait.min);
    productCollectionPage.facetFilter.filterByPriceToInput.type(`${price[30]}`).wait(Wait.min);
  ...
  it(`Should display '${statusMessage.noProducts}' when filter by price from ${price[0]} to ${price[0]}`, function () {
    productCollectionPage.facetFilter.filterByPriceButton.click();
    productCollectionPage.facetFilter.filterByPriceFromInput.type(`${price[0]}`).wait(Wait.min);
    productCollectionPage.facetFilter.filterByPriceToInput.type(`${price[0]}`).wait(Wait.min);
